    ...
    public class DamageUp : CardEffect {
        ...
        public override void UpdateDescription(Player _ = null) {
            this.Description = new[] {
                $"Increase {SpriteEffectMapping.Get(Effect.Damage)} by {this.Value}"
            };
            if (this.Duration != null) this.Description.AddRange(TurnsString(this.Duration.Value));
            // this.Description = $"{this.Value}{SpriteEffectMapping.Get(Effect.Damage, Modifier.Plus)}";
        ...
        private class Callback : OnCompute {
            ...
            public Callback(int value) : base(PRIORITY, CallbackType.Damage) => this.Value = value;
